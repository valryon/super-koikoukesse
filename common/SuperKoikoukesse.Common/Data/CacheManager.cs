...
using SuperKoikoukesse.Common.Exceptions;
using SuperKoikoukesse.Common.Log;
using System.Security;
using System.Runtime.CompilerServices;

namespace SuperKoikoukesse.Common.Data
{
    /// <summary>
        ...
        internal D Get<D>(String cacheKeyName)
        {
            ...
            if (cache.Contains(cacheKeyName) == false)
            {
                ...
                lock (getLock(cacheKeyName))
                {
                    if (cache.Contains(cacheKeyName) == false)
                    {
                        throw new CacheException("Unknow cache key: " + cacheKeyName);
                    }
                }
            }
            ...
            {
                throw new CacheException("Invalid cache object type. Parameter:" + typeof(D) + " but Cache object: " + data.GetType());
            }
        }
