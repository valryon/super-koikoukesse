    ...
    public class Grid
    {
        ...
        public void Draw(SpriteBatchProxy spriteBatch)
        {
            foreach (Vector2 loc in _grid.Keys)
            {
                ...
                if (GlitchApplication.GameStateManager.CurrentGameState.SceneCamera.VisibilityRectangle.Intersects(tile.DestRect))
                {
                    if (String.IsNullOrEmpty(tile.Spritesheet))
                    {
                        spriteBatch.Draw(GlitchApplication.MagicContentManager.EmptyTexture, tile.DestRect, tile.SourceRect, Color.Black);
                    }
                    else
                    {
                        spriteBatch.Draw(GlitchApplication.MagicContentManager.GetTexture(tile.Spritesheet), tile.DestRect, tile.SourceRect, Color.White);
                    }
                }
            }
        }
