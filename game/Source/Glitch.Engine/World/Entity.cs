...
namespace Glitch.Engine.World
{
    ...
    public abstract class Entity
    {
        ...
        public Entity(String assetName, Rectangle spriteRect)
            : this(assetName, spriteRect, Vector2.Zero, Vector2.One)
        {
        ...
        /// <param name="spriteRect"></param>
        public Entity(String assetName, Rectangle spriteRect, Vector2 location)
            : this(assetName, spriteRect, location, Vector2.One)
        {
        ...
        public Entity(String assetName, Rectangle spriteRect, Vector2 location, Vector2 scale)
        {
        ...
        public virtual void Draw(SpriteBatchProxy spriteBatch)
        {
            if (IsOnScreen())
            {
                spriteBatch.Draw(GlitchApplication.MagicContentManager.GetTexture(assetName), dRect, sRect, Color, (float)rotation, spriteOrigin, flip, 1.0f);

                if (GlitchApplication.IsDebugMode)
                {
                    if (hitbox != null)
                    {
        ...
        {
            var camera = GlitchApplication.GameStateManager.CurrentGameState.SceneCamera;
